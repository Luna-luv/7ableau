...
```
> **리이브**(실시간, 최신성 중요) : 데이터 원본의 변경사항이 태블로에 반영되어 실시간으로 정보가 업데이트
> **추출**(오프라인, 속도, 안정성 중요) : 데이터에 관계없이 원본 데이터에서 업데이트를 수신하려면 추출을 매번 새로 고쳐주어야 함
```
...
~~~
유니온
- 이유 : 데이터 필드명, 유형, 개수 모두 동일하고, 나눠서 분석하는 것이 아닌 5개 모두 시각화하고 싶은 것이기 때문에 그냥 데이터를 세로로 이어붙이는 유니온 사용 
~~~
...
```
라이브
- 이유: tableau 가 원본 데이터베이스에 직접 연결해서 최신 데이터 즉시 반영 가능 
```
